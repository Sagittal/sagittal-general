...
type Index<T = void> = Decimal<{ integer: true }> & { _IndexBrand: boolean } & (T extends void
        ? {}
        : { _IndexOfBrand: T })
type Count<T = void> = Multiplier & Decimal<{ integer: true }> & { _CountBrand: boolean } & (T extends void
        ? {}
        : { _CountOfBrand: T })
type Offset<T = void> = Decimal<{ integer: true }> & { _OffsetBrand: boolean } & (T extends void
        ? {}
        : { _OffsetOfBrand: T })

type Step<T extends NumericProperties & { of?: number } = {}> = // Iteration?
    Decimal<T> & { _StepBrand: boolean } & (T extends { of: number }
            ? { _StepOfEdBrand: T["of"] }
            : {})
type Ed<T extends NumericProperties & { of?: number } = {}> = // Generator?
    Decimal<T & { integer: true }> & { _EdBrand: boolean } & (T extends { of: number }
            ? { _EdOfWindowBrand: T["of"] }
            : {})
type Window<T extends NumericProperties & { of?: number } = {}> = // Period?
    Decimal<T> & { _WindowBrand: boolean } & (T extends { of: number }
            ? { _OfSizeBrand: T["of"] }
            : {})
