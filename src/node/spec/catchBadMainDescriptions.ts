    ...
    for (const file of fs.readdirSync(basePath)) {
        ...
        if (fs.lstatSync(filename).isDirectory()) {
            ...
            lines.forEach((line: Io): void => {
                const maybeDescribeMatches = line.match(/^describe\("(\w+)/)
                if (maybeDescribeMatches !== null)
                    subjectDescription = maybeDescribeMatches[INDEX_OF_CAPTURED_GROUP]

                const subjectActual = line.match(/\s*const actual = (?:await )?(\w+)/)
                if (
                    subjectActual !== null &&
                    ...
                    throw new Error(
                        `Mismatched main description and subject in module ${filename}: description says ${subjectDescription} but actual is ${subjectActual[INDEX_OF_CAPTURED_GROUP]}`,
                    )
                }
            })
        }
...
export { catchBadMainDescriptions }
