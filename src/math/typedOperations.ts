...
const isNegative = <T extends number>(number: T): boolean => number < 0

const isPositive = <T extends number>(number: T): boolean => number > 0
...
const round = <T extends number>(number: T, precision?: Precision): T => {
    ...
    return +(
        Math.round(`${String(number)}e+${String(precision)}` as unknown as number) +
        "e-" +
        String(precision)
    ) as T
}

const abs = <T extends number>(number: T): Abs<T> => Math.abs(number) as Abs<T>

const max = <T extends number>(...numbers: T[]): Max<T> => Math.max(...numbers) as Max<T>

const min = <T extends number>(...numbers: T[]): Min<T> => Math.min(...numbers) as Min<T>
