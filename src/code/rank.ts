...
const rank = <T>(arrayOfObjects: T[], options: RankOptions = {}): Array<T & { rank: Rank<T> }> => {
    const {
        ...
        strategy = RankStrategy.COMPETITION,
        descending,
        precision,
    } = options
    ...
    switch (strategy) {
        case RankStrategy.FRACTIONAL:
            return clonedArrayOfObjects.map((object: T, index: number): T & { rank: Rank<T> } => {
                ...
                clonedArrayOfObjects.slice(index + 1).forEach((objectWithWorseOrTiedRank: T): void => {
                    if (
                        isCloseOrEqual(
                            dig(objectWithWorseOrTiedRank as Obj, by),
                            dig(object as Obj, by),
                            precision,
                        )
                    ) {
                ...
                if (tiesCount === 0) {
                    ...
                    for (let i = index; i < index + tiesCount; i++) {
                        ;(clonedArrayOfObjects[i + 1] as T & { rank: Rank<T> }).rank = rank
                    }
        ...
        case RankStrategy.COMPETITION:
            return clonedArrayOfObjects.map((object: T): T & { rank: Decimal<Integer> & Rank<T> } => {
                ...
                if (isCloseOrEqual(rankingValue, previousValue, precision)) {
                    ...
                    return { ...object, rank: rank as Decimal<Integer> & Rank<T> }
                } else {
                    rank = (rank + 1 + tiesCount) as Decimal<Integer> & Rank<T>
                    tiesCount = 0 as Count
        ...
        case RankStrategy.ORDINAL:
            return clonedArrayOfObjects.map(
                (object: T, index: number): T & { rank: Decimal<Integer> & Rank<T> } => {
                    return { ...object, rank: (index + 1) as Decimal<Integer> & Rank<T> }
                },
            )
        default:
            throw new Error(`unknown rank strategy ${strategy as unknown as string}`)
    }
}
