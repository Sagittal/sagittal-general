...
const computeColumnWidths = <T>(table: Table<T>, columnRange: Range): Array<Count<Char>> =>
    columnRange.map((columnIndex: number): Count<Char> => {
        return table.reduce((columnWidth: Count<Char>, row: Row<{ of: T }>): Count<Char> => {
            ...
            const cellWidth =
                isUndefined(columnCell) || columnCell.includes("[/latex]")
                    ? (0 as Count<Char>)
                    : length(columnCell)
...
const alignCellIo = (cellIo: Maybe<Io>, { columnWidth, columnAlignment }: AlignedCellOptions): Io => {
...
export { computeColumnAlignments, computeColumnWidths, alignCellIo }
