...
describe("formatTableForTerminal", (): void => {
    const table = [
        ["comma name", "prime limit", "2,3-free SoPFR", "cents", "vector", "quotient", "apotome slope"],
    ...
    it("can align each column individually", (): void => {
        const tableAlignment = [Alignment.RIGHT, Alignment.LEFT, Alignment.CENTER, undefined, Alignment.RIGHT]
    ...
    it("can apply colors to the rows individually", (): void => {
        const colors: ColorMethod[] = ["cyan", "blue", "yellow"]
        ...
        const expected = ("comma name\tprime limit\t2,3-free SoPFR\tcents\tvector \tquotient\tapotome slope"
            .cyan.underline +
            NEWLINE +
            "11M       \t11         \t11            \t45.45\t[0 0 1⟩\t33/32   \t-4           ".blue +
            NEWLINE +
            "25/49M    \t7          \t24            \t33.4 \t[0 0⟩  \t50/49   \t-59.333      ".yellow +
            NEWLINE) as Io
        expect(actual).toEqual(expected)
    })
    ...
    it("can move the boundary between the header rows and the data rows", (): void => {
        const table = [
            ...
            ["name", "limit", "SoPFR", "cents", "vector", "quotient", "slope"],
            ["11M", "11", "11", "45.45", "[0 0 1⟩", "33/32", "-4"],
            ["25/49M", "7", "24", "33.4", "[0 0⟩", "50/49", "-59.333"],
        ] as Table<Io>
        ...
        const expected = ("comma \tprime\t2,3-free\t     \t       \t        \tapotome" +
            NEWLINE +
            "name  \tlimit\tSoPFR   \tcents\tvector \tquotient\tslope  ".underline +
            NEWLINE +
            "11M   \t11   \t11      \t45.45\t[0 0 1⟩\t33/32   \t-4     " +
            NEWLINE +
            "25/49M\t7    \t24      \t33.4 \t[0 0⟩  \t50/49   \t-59.333" +
